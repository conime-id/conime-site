    ...
    try {
      ...
      yamlBlock.split('\n').forEach(line => {
        ...
        if (idx !== -1) {
          ...
          let value = line.substring(idx + 1).trim().replace(/^["'](.*)["']$/, '$1');
          meta[key] = value;
        }
      });
    ...
    <div className="min-h-screen pb-20 animate-in fade-in duration-500">
      ...
      <div className="grid lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-8">
            ...
            <div className="bg-cogray-50 dark:bg-cogray-900/50 rounded-[40px] p-8 border border-cogray-100 dark:border-cogray-800 relative overflow-hidden">
               ...
               <div className="mt-8 pt-8 border-t border-cogray-100 dark:border-cogray-800 flex items-center justify-between">
                  ...
                  <button 
                    ...
                    onClick={async () => {
                        ...
                        try {
                          ...
                          setError('Gagal menghubungi helper server. Pastikan Antigravity sedang aktif.');
                        } finally {
                          setIsSaving(false);
                        }
                    }}
                  ...
                  >
                    ...
                    <span>{isSaving ? 'Menyimpan...' : 'Simpan Langsung'}</span>
                  </button>
               </div>
            </div>
          )}
          ...
          {success && (
            <div className="p-8 rounded-[40px] bg-conime-600 text-white shadow-2xl shadow-conime-600/30 animate-in zoom-in duration-500">
               <div className="flex items-center gap-6 mb-4">
                  <div className="w-16 h-16 rounded-3xl bg-white/20 flex items-center justify-center">
                    <CheckCircle className="w-10 h-10" />
                  </div>
                  <div>
                    <h3 className="text-xl font-black uppercase">Siap Disimpan!</h3>
                    <p className="text-white/80 text-sm font-medium">Beritahu Antigravity di chat: "Simpan file {file?.name}" untuk memasukkannya ke direktori konten.</p>
                  </div>
               </div>
            </div>
          )}
...
`;
                ...
                className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-cogray-800 border border-cogray-200 dark:border-cogray-700 rounded-xl text-xs font-bold uppercase tracking-widest hover:border-conime-600 transition-colors"
              >
         ...
         <div className="grid md:grid-cols-3 gap-8">
            <div className="space-y-3">
               ...
               <p className="text-xs text-cogray-500 leading-relaxed font-medium">Pastikan file .md memiliki header di antara baris --- (layout, title, category, dll). Gunakan template di atas.</p>
            </div>
            <div className="space-y-3">
               <div className="w-8 h-8 rounded-full bg-conime-600 text-white flex items-center justify-center font-black text-xs">2</div>
               <p className="text-sm font-bold text-cogray-800 dark:text-cogray-200 uppercase tracking-tight">Pemisah Bahasa</p>
               <p className="text-xs text-cogray-500 leading-relaxed font-medium">Gunakan baris --- saja untuk memisahkan konten ID (atas) dan EN (bawah).</p>
            </div>
            <div className="space-y-3">
               <div className="w-8 h-8 rounded-full bg-conime-600 text-white flex items-center justify-center font-black text-xs">3</div>
               <p className="text-sm font-bold text-cogray-800 dark:text-cogray-200 uppercase tracking-tight">Simpan</p>
               <p className="text-xs text-cogray-500 leading-relaxed font-medium">Klik tombol simpan untuk menulis file langung ke server lokal (pastikan server aktif).</p>
            </div>
         </div>
      </div>
    </div>
